...
export function UsersTable({session}: UsersTableProps) {
  ...
  const {data: users, error: usersError, isLoading: isUsersLoading} =
  ...
  const handleUserRoleChange = async (userId: string, newRole: UserRole) => {
    try {
      ...
      // 更新本地用户列表
      await mutate('/api/management/users', (currentUsers: UserAccount[] | undefined) =>
          ...
          <TableRow key={user.id}>
            ...
            <TableCell>
              <Button
                ...
                size="sm"
                className={cn(
                  user.enabled ? "bg-green-600 hover:bg-green-700": "bg-yellow-600 hover:bg-yellow-700"
                )}
