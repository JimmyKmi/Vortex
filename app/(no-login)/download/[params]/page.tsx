...
export default function DownloadPage({ params }: PageProps) {
  ...
  const { isActive, isValidating, transferInfo, checkSessionActive } = useTransferSession({
    sessionId
  })
  ...
  const handleDefaultDownload = async () => {
    ...
    try {
      ...
      for (let batchIndex = 0; batchIndex < actualSelectedFiles.length; batchIndex += batchSize) {
        ...
        // 更新状态显示
        setDownloadStatus(
          `正在生成 ${batchFileNames.join('、')}${batch.length < totalFiles ? '...' : ''}等 ${totalFiles} 个文件的下载通道`
        )
        ...
        // 开始下载文件
        setDownloadStatus(
          `正在下载 ${batchFileNames.join('、')}${batch.length < totalFiles ? '...' : ''} 等 ${totalFiles} 个文件`
        )
          ...
          <div className="flex flex-col-reverse items-stretch sm:flex-row justify-between gap-2 bg-muted/50 p-2 rounded-lg">
            <div className="flex gap-2 items-center">
              ...
              <span className="text-sm text-muted-foreground">
                共 {getTotalFileCount(files)} 个文件，已选择 {getSelectedFilesCount(files)} 个
              </span>
            </div>
          ...
          {currentDialogContent && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>{currentDialogContent.description}</DialogDescription>
              </DialogHeader>
              <DialogFooter className="gap-2 sm:gap-0">
                <Button variant="outline" onClick={() => dialogPromiseResolve?.(false)}>
                  取消
                </Button>
                <Button variant="default" onClick={() => dialogPromiseResolve?.(true)}>
